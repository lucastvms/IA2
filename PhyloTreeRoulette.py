...
def selection(reproduction_chance):
    ...
    for i in range(1, _FOREST_SIZE):
        reproduction_chance[i] += reproduction_chance[i - 1]

    for i in range(1, _FOREST_SIZE):
        if reproduction_chance[i - 1] < prob < reproduction_chance[i]:
            return i
...
def main():
    ...
    for i in range(_MAX_ITER):
        ...
        if best_fit_value < best_fit(species_fitness, random_forest)[0]:
            i -= 1
    ...
    for j in range(len(random_forest)):
        print(random_forest[j], ": ", reproduction_chance[j])
